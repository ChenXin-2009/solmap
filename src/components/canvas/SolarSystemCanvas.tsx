...
class SolarSystemRenderer {
  ...
  drawGradientOrbit(
    ...
    // 分段绘制，每段使用不同的透明度
    const segmentCount = 200; // 分段数量，越多渐变越平滑
    for (let seg = 0; seg < segmentCount; seg++) {
      const progress = seg / segmentCount; // 0 到 1
      ...
      // 计算下一段的点
      const nextProgress = (seg + 1) / segmentCount;
      const nextAngleOffset = nextProgress * trailLength;
      const nextAngle = startAngle + nextAngleOffset;
      const nextR = a * (1 - e * e) / (1 + e * Math.cos(nextAngle));
      const nextX_orb = nextR * Math.cos(nextAngle);
      const nextY_orb = nextR * Math.sin(nextAngle);
      
      const nextX = (cos_w * cos_O - sin_w * sin_O * cos_i) * nextX_orb +
                    (-sin_w * cos_O - cos_w * sin_O * cos_i) * nextY_orb;
      const nextY = (cos_w * sin_O + sin_w * cos_O * cos_i) * nextX_orb +
                    (-sin_w * sin_O + cos_w * cos_O * cos_i) * nextY_orb;
      
      const nextScreenX = this.centerX + (nextX + viewOffsetX) * zoom;
      const nextScreenY = this.centerY + (nextY + viewOffsetY) * zoom;
      
      // 渐变透明度：从当前位置（较不透明）到轨道末端（最低透明度）
      // 设置最低透明度，确保轨道始终可见
      const minOpacity = 0.15; // 最低透明度（15%）
      const maxOpacity = 0.8; // 最大透明度（80%）
      // 从maxOpacity渐变到minOpacity，使用平滑曲线
...
export default function SolarSystemCanvas() {
  ...
  }, [celestialBodies, selectedPlanet, viewOffset, zoom, lang, currentTime]);
